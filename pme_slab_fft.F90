...
#if Exclude
#ifdef MIC_offload
  integer               :: count_offload_task=0
    count_offload_task =0
#endif /*MIC_offload*/
#endif
...
#ifdef Exclude
#ifdef MIC_offload
   /*the exclusions of offloading MPI for recip task will be done if numtasks .gt 10*/
   /*currently only numtasks/2 and numtasks/2 -1 are the offloading MPI*/
    if(numtasks .gt. 10 .and. (taskid .eq. numtasks/2 .or. taskid .eq. numtasks/2 -1) )then
        xy_slab_cnt(taskid) = 0
        is_recip_task(taskid) = .false. ! un-mark recip tasks.
        count_offload_task = count_offload_task + 1
    end if
#endif /*MIC_offload*/
#endif
  end do
#ifdef Exclude
#ifdef MIC_offload
        if(count_offload_task .gt. 0) &
	recip_numtasks = recip_numtasks - count_offload_task
	!print*, "pme_slab_recip",recip_numtasks, count_offload_task
#endif /*MIC_offload*/
#endif
