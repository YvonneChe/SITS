...
! Local variables:
  ...
  do atm_i = 1, atm_cnt
    ...
    do sublst_idx = nxt_sublst + 1, nxt_sublst + excl_atm_j_cnt
      ...
      if (ti_mode .ne. 0) then
        call ti_update_nb_frc(-dfx, -dfy, -dfz, atm_i, ti_region)
      end if
#ifdef MPI
      if (owned_atm_j) then
#endif
        ...
        if (ti_mode .eq. 0) then
          ...
          call ti_update_nb_frc(dfx, dfy, dfz, atm_j, ti_region)
          ...
          if (ti_region .ne. 0) then
            ...
            ene_stk = ene_stk + cgi_cgj * d0 * ti_weights(ti_region)
            call ti_update_ene(cgi_cgj * d0, si_elect_ene, ti_region)
          else
            ene_stk = ene_stk + cgi_cgj * d0
          end if
        end if
