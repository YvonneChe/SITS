...
!                   gbl_my_atm_lst, new_list, .true., .false., pme_pot_ene, &
                   gbl_my_atm_lst, new_list, .true., .true., .false., pme_pot_ene, &
!-------------------------------------------------------------------------------
...
!                   new_list, .true., .false., pme_pot_ene, &
                   new_list, .true., .true., .false., pme_pot_ene, &
!-------------------------------------------------------------------------------
                   virial, ekcmt, pme_err_est)
#endif
#ifdef CUDA
...
#ifdef MPI
!Modified by Lijiang Yang
      call pme_force(atm_cnt, crd, frc, frc, frc, frc, img_atm_map, gbl_atm_img_map, &
...
! Partition energy terms:
  ...
  if (ti_mode .ne. 0) write(mdout, 9100)  si(si_dvdl)
