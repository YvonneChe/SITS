...
! support, but that we would have to support in a polarizable ff.
  ...
  write(sluene, 9221) nstep, time, temp
  write(sluene, 9229) si(si_pot_ene_slu)
  write(sluene, 9239) si(si_bond_ene_slu), si(si_angle_ene_slu), si(si_dihedral_ene_slu)
  write(sluene, 9249) si(si_vdw_14_ene_slu), si(si_elect_14_ene_slu), si(si_vdw_ene_slu)
  write(sluene, 9261) si(si_elect_ene_slu)
  write(solene, 9221) nstep, time, temp
  write(solene, 9229) si(si_pot_ene_sol)
  write(solene, 9239) si(si_bond_ene_sol), si(si_angle_ene_sol), si(si_dihedral_ene_sol)
  write(solene, 9249) si(si_vdw_14_ene_sol), si(si_elect_14_ene_sol), si(si_vdw_ene_sol)
  write(solene, 9261) si(si_elect_ene_sol)
  write(interene, 9221) nstep, time, temp
  write(interene, 9229) si(si_pot_ene_inter)
  write(interene, 9239) si(si_bond_ene_inter), si(si_angle_ene_inter), si(si_dihedral_ene_inter)
  write(interene, 9249) si(si_vdw_14_ene_inter), si(si_elect_14_ene_inter), si(si_vdw_ene_inter)
  write(interene, 9261) si(si_elect_ene_inter)
 !----------------------------------------------------------------------------
  ...
  if (si(si_dvdl) .ne. 0.d0) write(mdout, 9089) si(si_dvdl)
...
!  Printout SGLD guiding information
  ...
  if (remd_method .ne. 0 .and. iout7 .gt. 0 .and. remd_method .ne. -1) then
    if (remd_method .eq. 4) then
      ...
      if (isgld > 0)then
        write(mdout,9064) temp0,sgft,tempsg, replica_indexes(1),repnum,mdloop
      else
...
! If we flush the mdout, we also flush the cpout here if we're doing CpHMD
  ...
  if (first_6call) then
    ...
    if (isub .gt. 0) then
      close(sluene)
      open(unit=sluene, file=sluene_name, status='OLD', position='APPEND')
      close(solene)
      open(unit=solene, file=solene_name, status='OLD', position='APPEND')
      close(interene)
      open(unit=interene, file=interene_name, status='OLD', position='APPEND')
    end if
    !-----------------------------------------------------------------------
  else
...
!  Printout SGLD guiding information
  ...
  if (remd_method .ne. 0 .and. iout7 .gt. 0 .and. remd_method .ne. -1) then
    if (remd_method .eq. 4) then
      ...
      if (isgld > 0)then
        write(mdinfo,9064) temp0,sgft,tempsg, replica_indexes(1),repnum,mdloop
      else
...
!Added by Lijiang Yang
9221 format(/1x, 'NSTEP =', i9, 3x, 'TIME(PS) =', f12.3,2X, &
            'TEMP(K) =', f9.2)
9229 format(1x, 'EPtot  = ', f14.4)
9239 format(1x, 'BOND   = ', f14.4, 2x, 'ANGLE   = ', f14.4, 2x, &
            'DIHED      = ', f14.4)
9249 format(1x, '1-4 NB = ', f14.4, 2x, '1-4 EEL = ', f14.4, 2x, &
            'VDWAALS    = ', f14.4)
9261 format(1x, 'EELEC  = ', f14.4, 2x)
!-------------------------------------------------------------------------
