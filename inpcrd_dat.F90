...
module inpcrd_dat_mod
  ...
  double precision, allocatable, save   :: atm_slufrc(:,:)
  double precision, allocatable, save   :: atm_solfrc(:,:)
  double precision, allocatable, save   :: atm_interfrc(:,:)
  !-----------------------------------------------------------
...
! Open the inpcrd file:
  ...
  if (netcdf_input) then
    ...
    if (remd_method .eq. 1 .and. local_temp0 .ne. 1234321 &
        .and. irest .eq. 1) then
      write(mdout, '(a)') '| Overwriting temp0 from mdin with temp0 from &
                           &netcdf inpcrd file'
      temp0 = local_temp0
    else if (remd_method .eq. 4 .and. local_temp0 .ne. 1234321 &
             .and. irest .eq. 1) then
      write(mdout, '(a)') '| Overwriting temp0 from mdin with temp0 from &
                           &netcdf inpcrd file'
      solvph = local_temp0
  ...
  ! ---------- Formatted (ASCII) restart ----------
    ...
    if (remd_method .eq. 1 .and. local_temp0 .gt. 0 .and. irest .eq. 1) then
      write(mdout, '(a)') '| Overwriting temp0 from mdin with temp0 from &
                           &inpcrd file'
      temp0 = local_temp0
...
!Added by Lijiang Yang
           atm_slufrc(3, inpcrd_natom), &
           atm_solfrc(3, inpcrd_natom), &
           atm_interfrc(3, inpcrd_natom), &
!--------------------------------------------
           stat = alloc_failed)
...
!Added by Lijiang Yang
  atm_slufrc(:,:) = 0.d0           ! In case of nmr access to unowned atoms
  atm_solfrc(:,:) = 0.d0           ! In case of nmr access to unowned atoms
  atm_interfrc(:,:) = 0.d0           ! In case of nmr access to unowned atoms
!--------------------------------------------
#endif
