...
!*******************************************************************************
!
! Subroutine:  get_nb_energy_sub
!
! Description:
!              
! The main routine for non bond energy (vdw and hbond) as well as direct part
! of ewald sum.  It is structured for parallelism. Added by Lijiang Yang
!
!*******************************************************************************

subroutine get_nb_energy_sub(img_frc, img_crd, img_qterm, eed_cub, &
                         ipairs, tranvec, need_pot_enes, need_virials, &
           !              eed, evdw, ehb, eedvir, virial)
                         eed, evdw, ehb, eedvir, virial, img_atm_map,nsluatm,subindex)
...
!  vzz = 0.d0
  ...
  if (need_pot_enes) then
    ...
    do img_i = my_img_lo, my_img_hi
      ...
      if(subindex .eq. 2) then
        atm_i = img_atm_map(img_i)
        if(atm_i .le. nsluatm) cycle
      endif
      common_tran = ipairs(ipairs_idx)
      ipairs_idx = ipairs_idx + 1
...
end subroutine get_nb_energy_sub
